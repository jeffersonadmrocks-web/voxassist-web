...
(function(){
  ...
  async function renderAdmin(){
    ...
    app.innerHTML=`<div class="vx-admin-page company-only">
      <div class="vx-admin-hero"><div><button type="button" class="secondary vx-admin-back" id="vxAdminBack">← Voltar</button><h2>Empresas e Usuários</h2><p>Modo Empresa-only. Não existe Loja/Unidade operacional. Todo dado é isolado pela Empresa ativa.</p></div><div class="vx-active-company"><span>EMPRESA ATIVA</span><strong>${E(active?.trade_name||active?.legal_name||'NENHUMA EMPRESA CADASTRADA')}</strong></div></div>
