...
(function(){
  ...
  async function renderAdmin(){
    ...
    app.innerHTML=`<div class="vx-admin-page company-only">
      <div class="vx-admin-hero"><div><button type="button" class="secondary vx-admin-back" id="vxAdminBack">← Voltar</button><h2>Empresas e Usuários</h2><p>Empresa é o nível de isolamento de dados (multiempresa). Loja/Unidade é uma subdivisão administrativa dentro da empresa (cadastro em "Lojas", nesta mesma tela) -- usada na abertura de OS, agenda e estoque.</p></div><div class="vx-active-company"><span>EMPRESA ATIVA</span><strong>${E(active?.trade_name||active?.legal_name||'NENHUMA EMPRESA CADASTRADA')}</strong></div></div>
      ...
      <div class="vx-security-note"><b>ISOLAMENTO POR EMPRESA</b><span>OS, clientes, equipamentos, agenda, estoque, financeiro e demais dados usam company_id e RLS -- isolamento é sempre por Empresa. Loja é uma subdivisão dentro da empresa (cadastro em "Lojas"), não um segundo nível de isolamento.</span></div>
    </div>`;
  ...
  window.render=async function(view){
    ...
    if(view==='usuarios'){
      ...
      else if(section==='seguranca'){if(typeof window.renderSecuritySettings==='function'){await window.renderSecuritySettings();return;}}
