...
(function(){
  ...
  window.vxEditOrder=function(){
    ...
    card.innerHTML='<div style="font-size:15px;font-weight:700;color:#12324e;margin-bottom:5px">ALTERAR ORDEM DE SERVIÇO</div><div style="font-size:12px;color:#687b8e;margin-bottom:16px">Escolha o tipo de alteração.</div><button id="vxEditData" class="vx-action parts" style="width:100%;margin-bottom:8px">EDITAR DADOS DA O.S.</button><button id="vxEditStatus" class="vx-action" style="width:100%;margin-bottom:12px">ALTERAR SITUAÇÃO</button><div style="font-size:10px;color:#718399">Ao editar dados, navegue pelas abas normalmente e use o botão SALVAR para gravar tudo de uma vez.</div><div style="text-align:right;margin-top:15px"><button id="vxEditClose" class="vx-action">FECHAR</button></div>';
    ov.appendChild(card);document.body.appendChild(ov);
    const close=()=>ov.remove();card.querySelector('#vxEditClose').onclick=close;ov.onclick=e=>{if(e.target===ov)close();};
    card.querySelector('#vxEditData').onclick=()=>{close();document.querySelectorAll('.vx-os-panel [data-entity][data-name]').forEach(el=>{if(el.dataset.vxOriginalReadonly===undefined)el.dataset.vxOriginalReadonly=el.readOnly?'1':'0';if(el.dataset.vxOriginalDisabled===undefined)el.dataset.vxOriginalDisabled=el.disabled?'1':'0';if(el.dataset.vxOriginalReadonly!=='1')el.readOnly=false;if(el.dataset.vxOriginalDisabled!=='1')el.disabled=false;});toast('Modo de alteração ativado. Edite os dados necessários e clique SALVAR.');};
  ...
  window.manualStatus=function(){
    ...
    const current=String(o.status||'').replaceAll('_',' ');card.querySelector('#vxStatusCurrent').textContent='Situação atual: '+labelOf(current);const select=card.querySelector('#vxStatusSelect');FLOW.forEach(([value,label])=>{const op=document.createElement('option');op.value=value;op.textContent=label;if(value===current)op.selected=true;select.appendChild(op);});
    const reason=card.querySelector('#vxStatusReason'),hint=card.querySelector('#vxStatusHint');const idx=s=>FLOW.findIndex(x=>x[0]===String(s||'').replaceAll('_',' '));const needsReason=()=>{const a=idx(current),b=idx(select.value);return a<0||b<0||b<a||b>a+1||select.value==='ORCAMENTO RECUSADO';};const refreshHint=()=>{hint.textContent=needsReason()?'Esta mudança exige motivo para auditoria.':'Motivo opcional para esta mudança.';reason.style.borderColor=needsReason()?'#d79b46':'#bdc9d5';};select.onchange=refreshHint;refreshHint();card.querySelector('#vxStatusCancel').onclick=closeModal;overlay.onclick=e=>{if(e.target===overlay)closeModal();};
