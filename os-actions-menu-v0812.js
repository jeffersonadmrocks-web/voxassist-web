...
(function(){
  function ensure(){
    ...
    wrap.innerHTML=`
      <button type="button" class="vx-action" id="vxOsActionsBtn" aria-label="Abrir ações da OS" title="Ações" style="min-width:42px;padding:0 12px;font-size:16px;">▼</button>
      <div id="vxOsActionsDrop" style="display:none;position:absolute;right:0;top:calc(100% + 6px);z-index:10000;min-width:255px;background:#fff;border:1px solid #cbd7e3;box-shadow:0 8px 22px rgba(12,35,64,.14);padding:6px;">
        <div style="padding:7px 10px 4px;font-size:11px;color:#60738b;font-weight:700;">OPERAÇÃO</div>
        <button type="button" data-act="parts" style="width:100%;text-align:left;padding:10px;border:0;background:#fff;cursor:pointer;">Solicitar Peça</button>
    ...
    const btn=wrap.querySelector('#vxOsActionsBtn');
    const drop=wrap.querySelector('#vxOsActionsDrop');
    btn.onclick=e=>{
      e.stopPropagation();
    ...
    wrap.addEventListener('click',e=>{
      ...
      if(a.dataset.act==='parts') return parts?.click();
