...
(function(){
  ...
  const payStatusClass=s=>{const n=String(s||'').toUpperCase();if(['RECEBIDO','PAGO','QUITADO'].includes(n))return 'ok';if(['CANCELADO','CANCELADA','ESTORNADO','ESTORNADA'].includes(n))return 'off';return 'pending'};
  ...
  window.renderOsDetail=async function(id,tab='os'){
    ...
    ctx={o,c:o.clients||{},e:o.equipments||{},hist,parts,fin:finRows?.[0]||{},atts,payments,techs,phones,addresses,docs:filteredDocs,stores,clientOrders,clientEquipments,serviceGroups,partRequests,paymentMethods,activeTab:tab,selectedPayment:null,profileName:state.profile?.full_name||'USUÁRIO'};
