...
(function(){
  ...
  async function showWhirlpoolPanel(id){
    ...
    panel.innerHTML=`<div class="vx-screen-box"><div class="vx-wp-head"><div><h3>MODO WHIRLPOOL • BRASTEMP / CONSUL ${typeof window.vxStatusBadge==='function'?window.vxStatusBadge('EM HOMOLOGAÇÃO','Salvar e imprimir já são reais; a consolidação visual dos ajustes de layout está em andamento.'):''}</h3><small>Extensão da OS VoxAssist. Dashboard, Agenda, Financeiro e Histórico continuam usando a mesma OS.</small></div><button class="vx-action" id="vxWpPrint">IMPRIMIR DOCUMENTO WHIRLPOOL</button></div>
