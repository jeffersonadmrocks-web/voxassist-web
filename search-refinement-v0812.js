/* VoxAssist Web V0.8.12 — buscas refinadas sem observer global */
(function(){
  ...
  function enhanceOrders(){
    const input=document.querySelector('#osSearch');
    if(!input||input.dataset.refined)return;
    input.dataset.refined='1';
    input.placeholder='Pesquisar OS, cliente, CPF/CNPJ, telefone, marca, modelo, série, técnico ou situação';
    input.value=searchState.q;
    const toolbar=input.closest('.toolbar')||input.parentElement;
    if(!toolbar||document.querySelector('.vx-search-filters'))return;
    const filter=document.createElement('div');filter.className='vx-search-filters';
    ...
    filter.querySelectorAll('[data-sf]').forEach(s=>s.onchange=()=>{searchState[s.dataset.sf]=s.value;apply()});
    filter.querySelector('#clearSearchFilters').onclick=()=>{Object.assign(searchState,{q:'',status:'',brand:'',group:'',tech:'',service:''});input.value='';filter.querySelectorAll('select').forEach(s=>s.value='');apply()};
    apply();
  }
  ...
  function datalist(id,values){
    ...
    const html=uniq(values).map(v=>`<option value="${esc(v)}"></option>`).join('');
  ...
  function enhanceAutocompletes(){
    ...
    if(brand&&model&&!brand.dataset.modelFilter){brand.dataset.modelFilter='1';brand.addEventListener('input',()=>{const b=norm(brand.value);datalist('vx-models',state.orders.filter(o=>!b||norm(o.equipments?.brand)===b).map(o=>o.equipments?.model))})}
  }
  ...
  function enhanceCurrent(){requestAnimationFrame(()=>{enhanceOrders();enhanceAutocompletes()})}
  window.render=async function(view){const out=await oldRender(view);enhanceCurrent();return out};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',enhanceCurrent,{once:true});else enhanceCurrent();
})();
