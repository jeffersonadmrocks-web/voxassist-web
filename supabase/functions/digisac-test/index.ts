...
function sanitized(body: Record<string, unknown>, status: number) {
  return new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json" } });
}

Deno.serve(async (req) => {
  if (req.method !== "POST") {
    return sanitized({ ok: false, error: "method_not_allowed" }, 405);
  }

  const authHeader = req.headers.get("Authorization");
  if (!authHeader) {
    return sanitized({ ok: false, error: "unauthorized" }, 401);
  }
  ...
  // Só GESTOR (papel de gestão/administração do VoxAssist) pode disparar o
  // teste — requisito explícito do pedido, evita qualquer usuário comum
  // sondar a integração.
  if (!isAuthorizedRole(profile?.role ?? null)) {
    return sanitized({ ok: false, error: "forbidden" }, 403);
  }
  ...
  try {
    ...
    // sanitizada de classifyNetworkError/classifyResponse sai da function).
    ...
    // Best-effort: só preenche se o corpo (já sanitizado — sem token/headers)
    // trouxer algo parecido com nome de conta/usuário. A rota testada é de
    // baixo risco (agenda), então normalmente estes campos ficam null — é
    // esperado nesta etapa e documentado no relatório.
