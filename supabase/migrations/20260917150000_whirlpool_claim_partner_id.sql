-- Achado real (2026-09-17): a varredura automática de "Pesquisa: ordens
-- de serviço" roda com todos os filtros em branco (só Nº máximo
-- resultados=1000) e sempre volta com a grade renderizada mas 0 linhas
-- -- confirmado via diagnóstico (thead correto, 4 colunas batem, tbody
-- vazio -- não é bug de leitura da grade, ver whirlpool_worker_claim
-- anterior). O usuário confirmou por print real: uma busca preenchendo
-- "ID do parceiro de negócios" retorna a grade cheia (paginação até a
-- página 100). A coluna external_partner_id já existe em
-- whirlpool_connections desde a fundação (20260914193000) exatamente
-- para isto, mas nunca tinha sido devolvida pelo claim -- o worker não
-- tinha como preenchê-la porque nunca a recebia. Só acrescenta ao jsonb
-- já existente (mesmo padrão do claim_scan_context anterior); nenhuma
-- lógica de claim muda.
