-- Achado real do usuário (2026-09-22, run 35725302826, ~5h após a
-- correção da trava de login): "PORTAL_INDISPONIVEL — Gateway Whirlpool
-- falhou (HTTP 400): duplicate key value violates unique constraint
-- whirlpool_import_queue_open_unique". Índice e função já foram
-- confirmados IDÊNTICOS ao repositório (pg_get_functiondef/pg_indexes
-- reais de produção conferidos pelo usuário), e reproduções locais de
-- concorrência real (10 chamadas simultâneas ao mesmo item, item
-- duplicado no mesmo array) nunca lançaram exceção -- descartando
-- índice divergente, função divergente e corrida entre INSERTs.
--
-- Causa raiz real, reproduzida em Postgres local: o índice parcial
-- whirlpool_import_queue_open_unique NUNCA incluiu
-- AGUARDANDO_CONEXAO_WHIRLPOOL na lista de estados protegidos. Toda vez
-- que o portal fica indisponível, whirlpool_worker_report move TODAS as
-- linhas PENDENTE/PROCESSANDO em massa para esse estado -- que fica
-- FORA do índice, sem proteção de unicidade. Como o portal ficou
-- indisponível repetidamente nesta semana (tela "System Messages",
-- coluna CONCATSTATUSER, trava de login), o scan reinseriu a mesma OS
-- várias vezes, criando duplicatas físicas reais na tabela. Só nunca
-- apareceu antes porque a conexão nunca "voltava" de verdade -- a
-- correção da trava de login (2026-09-21) finalmente permitiu isso, e
-- o UPDATE em massa que reabre AGUARDANDO_CONEXAO_WHIRLPOOL -> PENDENTE
-- (um UPDATE simples, sem ON CONFLICT) quebra ao mover duas duplicatas
-- pro mesmo estado na mesma linha do plano de execução.
--
-- Corrige em duas partes, na ordem que importa:
-- 1) Deduplica o que já existe (mantém a linha mais recente de cada
--    (external_order_id, queue_reason) entre os estados "abertos",
--    move as demais para IGNORADO com o motivo registrado) -- sem isso
--    o DROP/CREATE INDEX abaixo falharia contra dados já duplicados.
-- 2) Recria o índice único (e o ON CONFLICT de whirlpool_ingest_catalog,
--    que precisa bater exatamente com o índice) incluindo
--    AGUARDANDO_CONEXAO_WHIRLPOOL na lista de estados protegidos --
--    nunca mais permite que essa duplicação se acumule.
--
-- Validado em Postgres real (schema completo reconstruído a partir das
-- migrations existentes): reproduzido o erro exato com duas linhas em
-- AGUARDANDO_CONEXAO_WHIRLPOOL para a mesma OS/motivo, confirmada a
-- deduplicação e confirmado que o mesmo UPDATE em massa que antes
-- quebrava agora funciona sem erro; confirmado que whirlpool_ingest_
-- catalog continua sem duplicar (queued:0) quando a OS já está em
-- AGUARDANDO_CONEXAO_WHIRLPOOL.

with duplicatas as (
  select id,
    row_number() over (
      partition by external_order_id, queue_reason
      order by created_at desc
    ) as rn
  from public.whirlpool_import_queue
  where state in ('PENDENTE','PROCESSANDO','AGUARDANDO_OPERADOR','AGUARDANDO_CONEXAO_WHIRLPOOL','ERRO')
)
update public.whirlpool_import_queue q
set state = 'IGNORADO',
    last_error_message = 'Duplicata da fila consolidada automaticamente (migration 20260922130000 -- mesma OS/motivo já tinha outra entrada aberta mais recente)',
    updated_at = now()
from duplicatas d
where q.id = d.id and d.rn > 1;

drop index if exists public.whirlpool_import_queue_open_unique;
create unique index whirlpool_import_queue_open_unique
