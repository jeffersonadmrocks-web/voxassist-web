...
(function(){
  ...
  function labelFor(v,fallback){
    if(String(v).startsWith('os:')){
      const id=String(v).split(':')[1];
      const order=(state.orders||[]).find(o=>String(o.id)===id);
      const number=order?.os_number || (state.activeOs && String(state.activeOs.id)===id ? state.activeOs.os_number : '');
  ...
  function replaceActiveTab(next){
    ...
    if(state.openTabs.length===0){state.openTabs=[next];idx=0;}else{state.openTabs[idx]=next;}
    state.view=next;
  }
  ...
  window.render=async function(view){
    ...
    if(switchingExistingTab){
      state.view=view;if(!state.openTabs.includes(view))state.openTabs.push(view);
      if(view.startsWith('guide:')){renderTabs();const title=document.querySelector('#title');if(title)title.textContent='Dashboard';document.querySelectorAll('.desktop-menu .nav').forEach(b=>b.classList.toggle('active',b.dataset.view==='dashboard'));if(typeof window.renderDashboard==='function')return window.renderDashboard();return previousRender('dashboard');}
      const out=await previousRender(view);if(document.querySelector('#tabs'))renderTabs();return out;
    }
    if(!view.startsWith('guide:')) replaceActiveTab(view);
    if(view.startsWith('guide:')){state.view=view;if(!state.openTabs.includes(view))state.openTabs.push(view);renderTabs();const title=document.querySelector('#title');if(title)title.textContent='Dashboard';document.querySelectorAll('.desktop-menu .nav').forEach(b=>b.classList.toggle('active',b.dataset.view==='dashboard'));if(typeof window.renderDashboard==='function')return window.renderDashboard();return previousRender('dashboard');}
    const out=await previousRender(view);
    const seen=new Set();state.openTabs=state.openTabs.filter(v=>{if(seen.has(v))return false;seen.add(v);return true;});state.view=view;if(document.querySelector('#tabs'))renderTabs();return out;
  };
})();
