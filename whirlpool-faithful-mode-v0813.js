...
(function(){
  ...
  async function printFaithful(id){
    ...
    <table class="t"><tr><td colspan="2">CONSUMIDOR: ${E(data.consumidor)}</td><td>CEP: ${E(data.cep)}</td><td>REGIÃO: ${E(data.regiao)}</td></tr><tr><td colspan="2">ENDEREÇO: ${E(data.endereco)}</td><td colspan="2">BAIRRO: ${E(data.bairro)}</td></tr><tr><td colspan="2">COMPLEMENTO: ${E(data.complemento)}</td><td>CIDADE: ${E(data.cidade)}</td><td>UF: ${E(data.uf)}</td></tr><tr><td colspan="2">CNPJ/CPF: ${E(data.cnpjCpf)}</td><td colspan="2">ENDEREÇO ELETRÔNICO: ${E(data.enderecoEletronico)}</td></tr><tr><td colspan="4">${E(tel)}</td></tr><tr><td colspan="4">LOCALIZAÇÃO: ${E(data.localizacao)}</td></tr></table>
    <table class="t"><tr><td colspan="2">PRODUTO: ${E(data.produto)}</td><td colspan="2">MARCA: ${E(data.marca)}</td></tr><tr><td colspan="2">PRODUTO CONSUMIDOR: ${E(data.produtoConsumidor)}</td><td colspan="2">LINHA: ${E(data.linha)}</td></tr><tr><td>SÉRIE: ${E(data.serie)}</td><td>NOME COMERCIAL: ${E(data.nomeComercial)}</td><td colspan="2">TEMPO DE USO: ${E(data.tempoUso)}</td></tr><tr><td colspan="4">TIPO DE OS: ${E(data.tipoOS)}</td></tr><tr><td colspan="4">NR NOTA FISCAL: ${E(data.nrNotaFiscal)} &nbsp;&nbsp; DATA COMPRA: ${E(data.dataCompra)} &nbsp;&nbsp; COR: ${E(data.cor)} &nbsp;&nbsp; VOLTAGEM: ${E(data.voltagem)} &nbsp;&nbsp; CAPACIDADE: ${E(data.capacidade)}</td></tr></table>
    ...
    <div class="box term"><div class="title">TERMO DE GARANTIA DO SERVIÇO AUTORIZADO</div><p>CONFORME DESCRITO NO ORÇAMENTO JÁ APROVADO, FIRMAMOS A GARANTIA DO SERVIÇO (MÃO DE OBRA) DE ASSISTÊNCIA TÉCNICA POR UM PERÍODO DE _______(___) MESES E DAS PEÇAS APLICADAS POR UM PERÍODO DE _______(___) MESES, A PARTIR DE ________________ (DATA DE CONCLUSÃO), QUANDO O SERVIÇO FOI DEVIDAMENTE EXECUTADO, ESTANDO EM PERFEITAS CONDIÇÕES DE UTILIZAÇÃO, TENDO RECEBIDO AS ORIENTAÇÕES NECESSÁRIAS PARA A CORRETA UTILIZAÇÃO DO PRODUTO.</p><p>EXCLUEM-SE DA GARANTIA OS DEFEITOS CAUSADOS POR USO IMPRÓPRIO OU INADEQUADO DO PRODUTO E PROBLEMAS DECORRENTES DE ACIDENTES NATURAIS.</p><p>DE ACORDO.</p></div>
