...
(function(){
  ...
  // PWA-1 (2026-09-13) -- achado do usuário: OS real (7015285017)
  // imprimindo em 2 páginas em vez de 1 (regressão do que a task
  // "Refinar layout de impressão da OS Whirlpool" original garantiu).
  // Medido com Playwright (não suposto): o conteúdo real dessa OS
  // renderiza a ~308.9mm de altura, ~22mm além dos 287mm disponíveis
  // (297mm de A4 menos 5mm+5mm de margem) -- gerava mesmo uma 2ª
  // página, sem bug de renderização nenhum, só falta de orçamento
  // vertical. .parts td sempre desenha no mínimo 8 linhas em branco
  // (Math.max(8,parts.length) mais abaixo) mesmo numa OS sem peças
  // lançadas, então essas linhas (e o padding de toda célula do
  // documento) são o alvo mais seguro pra cortar -- é espaço em
  // branco puro, não conteúdo. Reduzido padding:1.5mm->1.1mm e altura
  // da linha de peças 6mm->5mm (~22mm economizados no total, testado
  // e confirmado 1 página com esta mesma OS). Nenhum tamanho de fonte
  // de conteúdo real, texto ou estrutura mudou.
  async function printFaithful(id){
    ...
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>Whirlpool ${E(data.numeroOS)}</title><style>@page{size:A4;margin:5mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#000;font-size:7.5pt}.a4{width:200mm;margin:auto}.t{width:100%;border-collapse:collapse;margin:0}.t td,.t th{border:1px solid #000;padding:1.1mm;vertical-align:top}.t th{font-weight:700;text-align:center}.lbl{font-size:6.5pt;text-align:center}.strong{font-weight:700}.multi{height:13mm}.parts td{height:5mm}.noinner{border:1px solid #000}.noinner td{border:none!important}.box{border:1px solid #000;padding:2mm;margin-top:0}.title{text-align:center;font-weight:700}.auth{min-height:31mm}.term{font-weight:700;font-size:6.5pt;text-align:justify}.sig{height:16mm;max-width:70mm;border-bottom:1px solid #000;object-fit:contain}.nowrap{white-space:nowrap}</style></head><body><div class="a4">
