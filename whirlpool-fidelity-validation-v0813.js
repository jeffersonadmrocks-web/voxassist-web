...
(function(){
 ...
 async function run(){const form=$('#vxWpForm');if(!form||form.closest('.hidden')||!$('.wpf-doc',form))return;style();validate(form);await integrateSignatures(form);form.querySelectorAll('input,textarea').forEach(el=>{if(!el.dataset.fidelityWatch){el.dataset.fidelityWatch='1';el.addEventListener('input',()=>validate(form))}})}
